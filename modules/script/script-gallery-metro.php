	...
	if(jQuery('#tg_portfolio_filterable_link').val()!=1)
	{
		jQuery('#portfolio_wall_filters_<?php echo esc_attr($_GET['id']); ?> li a, #portfolio_wall_filters li a').click(function(){
		  	...
		  	jQuery.ajax({
		        ...
		        type:'POST',
				cache: true,
		        data: 'action=<?php echo esc_js($_GET['next_action']); ?>&cat='+selector+'&items=-1&layout=<?php echo esc_js($_GET['layout']); ?>&current_page_id=<?php echo esc_js($_GET['page_id']); ?>&tg_security='+tgParams.ajax_nonce, 
