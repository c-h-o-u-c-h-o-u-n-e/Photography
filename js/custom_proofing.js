	...
	jQuery('.image_approve').click(function(){
		...
		if(jQuery('#gallery_proofing_status').val() == 0)
		{
			...
			jQuery.ajax({
		        ...
		        type:'POST',
		        data: 'action=photography_image_proofing&method=approve&gallery_id='+galleryID+'&image_id='+imgID+'&author_id='+authorID+'&tg_security='+tgParams.ajax_nonce, 
	...
	jQuery('.image_unapprove').click(function(){
		...
		if(jQuery('#gallery_proofing_status').val() == 0)
		{
			...
			jQuery.ajax({
		        url: tgParams.ajaxurl,
		        type:'POST',
		        data: 'action=photography_image_proofing&method=unapprove&gallery_id='+galleryID+'&image_id='+imgID+'&author_id='+authorID+'&tg_security='+tgParams.ajax_nonce, 
		        success: function(html)
		        {
